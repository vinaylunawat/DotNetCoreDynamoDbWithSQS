namespace Framework.Business.Extension
{
    ...
    public static class IndexedItemsExtensions
    {
        ...
        public static ErrorRecord<TKey, TErrorCode> CreateErrorMessage<TKey, T, TErrorCode>(this IIndexedItem<T> indexedItem, string propertyName, TErrorCode errorCode, string message, object attemptedValue)
            where TErrorCode : struct, Enum
        {
            var model = indexedItem.Item as IModelWithKey<TKey>;
            var errorMessage = new ErrorMessage<TErrorCode>(propertyName, errorCode, message, attemptedValue);
            if (model is null)
            {
                return new ErrorRecord<TKey, TErrorCode>(indexedItem.OrdinalPosition, errorMessage);
            }
            else
            {
                return new ErrorRecord<TKey, TErrorCode>(model.Id, indexedItem.OrdinalPosition, errorMessage);
            }
        }
    }
}
